...
SCENARIO("Product"){
  GIVEN("one argument"){
    THEN("the product type is always defined"){
      constexpr auto correctTypes =
        std::is_same<shacl::trait::Product_t<Does>, Does>::value
        and std::is_same<shacl::trait::Product_t<Doesnt>, Doesnt>::value;
      REQUIRE(correctTypes);
    }
  }
  GIVEN("a pair of parameters"){
    GIVEN("multiplication is defined over the pair of type"){
      THEN("product provides an alias to the resulting type"){
        {
          constexpr auto correctType =
            std::is_same<shacl::trait::Product_t<Does, Does>,
                         Does>::value;
          REQUIRE(correctType);
        }{
          constexpr auto correctType =
            std::is_same<shacl::trait::Product_t<OneWay<2>, OneWay<3>>,
                         OneWay<1>>::value;
          REQUIRE(correctType);
        }
      }
    ...
    GIVEN("multiplication is NOT defined over the pair of type"){
      THEN("product does not provide a nested type alias"){
        {
          constexpr auto hasNestTypeAlias =
            shacl::trait::HasType_v
            <shacl::trait::Product<Doesnt, Doesnt>>;
          REQUIRE(not hasNestTypeAlias);
        }{
          constexpr auto hasNestTypeAlias =
            shacl::trait::HasType_v
            <shacl::trait::Product<OneWay<3>, OneWay<2>>>;
          REQUIRE(not hasNestTypeAlias);
        }
      }
    }
  ...
  GIVEN("a more than 2 parameters"){
    GIVEN("multiplication is defined over a sequence of type"){
      THEN("product provides an alias to the resulting type"){
        {
          constexpr auto correctType =
            std::is_same<shacl::trait::Product_t<Does, Does, Does>, Does>::value;
          REQUIRE(correctType);
        }{
          constexpr auto correctType =
            std::is_same<shacl::trait::Product_t<OneWay<2>, OneWay<3>, OneWay<4>>,
                         OneWay<3>>::value;
          REQUIRE(correctType);
        }
      }
      ...
      THEN("product does not provide a nested type alias"){
        {
          constexpr auto hasNestTypeAlias =
            shacl::trait::HasType_v
            <shacl::trait::Product<int, int, Doesnt>>;
          REQUIRE(not hasNestTypeAlias);
        }{
          constexpr auto hasNestTypeAlias =
            shacl::trait::HasType_v
            <shacl::trait::Product<OneWay<2>, OneWay<5>, OneWay<2>>>;
          REQUIRE(not hasNestTypeAlias);
        }{
          constexpr auto hasNestTypeAlias =
            shacl::trait::HasType_v
            <shacl::trait::Product<OneWay<5>, OneWay<2>, OneWay<4>>>;
          REQUIRE(not hasNestTypeAlias);
        }
      }
    }
